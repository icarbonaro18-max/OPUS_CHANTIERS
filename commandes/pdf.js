...
export async function makePDF(order,{logo,supplierLogo='',project=''}={}){
 ...
 line('BON DE COMMANDE',true,20);line('Préparation et récupération du matériel',false,10);y+=4;
 ...
 line(order.deliveryMode==='retrait'?'RETRAIT EN AGENCE':'LIVRAISON',true,11);line(order.destination);
 if(order.availability)line('Suivi fournisseur : '+order.availability);
 ...
 for(let i=1;i<=p.getNumberOfPages();i++){p.setPage(i);p.setFontSize(8);p.setTextColor(100);p.text('OPUS ELEC · Document interne établi à partir de la commande fournisseur',18,283);p.text(`${i} / ${p.getNumberOfPages()}`,192,283,{align:'right'});p.text('Ne remplace pas la confirmation ni le bon de livraison du fournisseur.',18,288);}
