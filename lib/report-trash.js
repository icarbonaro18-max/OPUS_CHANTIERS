...
export async function setReportDeleted(ui,report,deleted,collection='interventions'){
 if(!ui.c.isAdmin?.())throw Error('Suppression réservée à l’administrateur.');
 if(!['interventions','visits'].includes(collection))throw Error('Type de compte rendu inconnu.');
 ...
 try{await ui.save(collection);}catch(error){Object.assign(report,before);throw error;}
}
