...
export async function setInterventionArchived(ui,id,archive=true){
 if(!ui.c.isAdmin())throw Error('Classement réservé à l’administrateur.');
 if(ui.dataLoadError)throw Error('Rechargez les données Microsoft avant le classement.');
 const g=ui.c.graph,{meta,rows}=await currentInterventions(g);
 const matches=rows.filter(r=>r.id===id);
 const record=matches[0];
 if(matches.length!==1||!isIndependentIntervention(record)||!['terminee','facturee'].includes(record.status))throw Error('Seule une intervention achevée peut être archivée. Actualisez la liste.');
 const now=new Date().toISOString();
 Object.assign(record,{archivedAt:archive?(record.archivedAt||now):null,archivedBy:archive?(ui.user?.displayName||''):null,updatedAt:now});
 ...
 const confirmed=await g.json(meta.id),saved=Array.isArray(confirmed)&&confirmed.find(r=>r.id===id);
 if(!saved||saved.archivedAt!==record.archivedAt)throw Error('Classement non confirmé par Microsoft. Actualisez avant de réessayer.');
