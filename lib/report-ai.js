...
export function bindReportAI(ui,x,canEdit,visit=false) {
  ...
  form.addEventListener('input',ev=>{if(!section.contains(ev.target)&&generatedSource!==fingerprint()){approve.checked=false;status.textContent='Notes modifiées : régénérez le rapport avant validation.';}});
