...
export function readerPlanning(events,people,from,to){
 return events.filter(e=>['project','intervention','visit'].includes(e.kind)&&e.status!=='annulee').flatMap(e=>{
  ...
  const team=[...new Set(e.teamIds||[])].flatMap(id=>{
   ...
   if(p.type!=='external'&&!weekend&&a.date===b.date){start=start<'08:00'?'08:00':start;const finish=friday?(p.role==='apprenti'||Number(p.weeklyTarget)===35?'11:00':'16:00'):'17:00';end=end>finish?finish:end;if(end<=start)return [];}
