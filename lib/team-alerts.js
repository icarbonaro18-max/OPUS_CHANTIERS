...
export function mountTeamAlerts(ui){
 const host=document.getElementById('mainNav');if(!host||document.getElementById('teamAlerts'))return;
 ...
 const badge=()=>{const n=unreadAlerts(rows,seen).length;button.innerHTML='Nouveautés '+(n?'<span class="unreadBadge">'+n+'</span>':'');button.setAttribute('aria-label',n+' nouveautés non lues');if(error)button.append(' · À vérifier');};
