...
export async function savePeriods(ui,base,periods){
 ...
 try{ui.data.events=[...before];for(const p of periods){const days=periodDays(p.from,p.to,p.weekends);if(!p.start||!p.end||p.end<=p.start||!Number.isFinite(p.pause)||p.pause<0)throw Error('Vérifiez les horaires et la pause.');for(const date of days){ui.validateScheduling(date,p.start,p.end,p.teamIds,'');const start=new Date(date+'T'+p.start).toISOString(),end=new Date(date+'T'+p.end).toISOString();if(p.pause>=durationHours(start,end,0))throw Error('La pause doit être inférieure à la durée de présence.');const e={...base,id:makeId('evt'),seriesId,start,end,teamIds:[...new Set(p.teamIds)],pauseHours:p.pause};e.teamSize=e.teamIds.length||base.teamSize;e.laborHours=durationHours(start,end,p.pause)*e.teamSize;staged.push(e);ui.data.events.push(e);}}
