...
export function validateInterventionDays(ui,x,start,end,team,weekends=false){
 ...
 for(const row of rows){try{ui.validateScheduling(row.date,row.start,row.end,team);}catch(e){throw Error(row.date+' : '+e.message);}}
...
export async function saveInterventionDays(ui,x){
 ...
 const linked=before.filter(e=>e.kind==='intervention'&&e.linkId===x.id);
 ...
 ui.data.events=[...before.filter(e=>!(e.kind==='intervention'&&e.linkId===x.id)),...events];
