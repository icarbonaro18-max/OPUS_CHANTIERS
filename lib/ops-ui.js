import {reassignPlanning,movableProject,originalAppointment,calendarEvents} from './planning-reassign.js';
...
export class OpsUI{
  ...
  async editIntervention(existing=null){
    ...
    if(existing&&this.canPlan())document.getElementById('deleteInt').onclick=async()=>{if(!confirm(`Supprimer ${x.number} ?`))return;this.data.interventions=this.data.interventions.filter(r=>r.id!==x.id);this.data.events=reassignPlanning(this.data.events,[],this.data.events.filter(r=>r.kind==='intervention'&&r.linkId===x.id).map(r=>r.id),this.data.people);await Promise.all([this.save('interventions'),this.save('events')]);document.getElementById('modal').close();await this.renderInterventions();this.c.toast('Intervention supprimée.');};
  }
  ...
  async renderCalendar(){
    ...
    root.innerHTML=`<div class="pageHeading"><div><p class="eyebrow">PLANNING PARTAGÉ</p><h1>Calendrier des interventions</h1><p class="muted">Vue calendrier : chantier, intervention ou visite. Les apprentis à l’école et les absences restent bloquantes. Une intervention peut remplacer un créneau de chantier.</p></div><div class="actionRow">${this.canPlan()?'<button id="planEvent">+ Planifier</button><button id="resourcesBtn" class="secondary">Équipe & ressources</button>':''}</div></div>
  ...
  async planEvent(existing=null,slot=null){
    ...
    const form=document.getElementById('eventForm');form.onsubmit=async ev=>{ev.preventDefault();await this.saveWithFeedback(form,existing?'Planning modifié ✓':'Planning enregistré ✓',async()=>{
      ...
      const exactHours=document.getElementById('eventExactHours').checked;this.validateScheduling(date,start,end,selectedTeamIds,e.id,exactHours,kind==='intervention');e.exactHours=exactHours;
