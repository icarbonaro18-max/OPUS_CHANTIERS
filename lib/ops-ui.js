...
export class OpsUI{
  ...
  sortProjects(rows){return [...rows].sort((a,b)=>{const pa=['04','99'].includes(a.category)?{today:false,event:null}:projectPriority(this.data.events,a.id),pb=['04','99'].includes(b.category)?{today:false,event:null}:projectPriority(this.data.events,b.id);if(pa.today!==pb.today)return pa.today?-1:1;if(pa.event&&pb.event)return pa.event.start.localeCompare(pb.event.start);if(pa.event)return -1;if(pb.event)return 1;return 0;});}
  ...
  openProjectReports(p,meta){return projectReports(this,p,meta);}
  ...
  async interventionPdf(x,{visit=false}={}){
    ...
    const header=()=>{
      ...
      pdf.text(visit?"VISITE AVANT DEVIS":"RAPPORT D’INTERVENTION",W-M,18,{align:'right'});
      ...
      pdf.text('COMPTE RENDU CLIENT',W-M,31,{align:'right'});
    ...
    txt(x.siteAddress||'Adresse non renseignée',9);if(x.devis)txt(`Devis : ${x.devis}`,9,true);
    ...
    if((x.photos||[]).length){section('PHOTOS GÉNÉRALES');(x.photos||[]).forEach((ph,j)=>addPhoto(ph.dataUrl,`Photo générale ${j+1}`));}
  ...
  async saveVisitPackage(v){
    ...
    const saved={...v,pdfSavedAt:new Date().toISOString(),pdfFolder:`VISITES_AVANT_DEVIS/${year}/${folderName}`};
  ...
  async saveInterventionPackage(x,{download=false}={}){
    ...
    await g.request(g.base(dir.id)+':/'+encodeURIComponent(pdfName)+':/content',{method:'PUT',headers:{'Content-Type':'application/pdf'},body:blob});
    const clean=structuredClone(x);await g.writeJson(dir.id,'intervention.json',clean);x.pdfSavedAt=new Date().toISOString();x.pdfFolder=x.projectId?`${x.projectName}/05_PDF_GENERES/COMPTES_RENDUS/${folderName}`:`INTERVENTIONS/${year}/${folderName}`;await this.save('interventions');
  ...
  async editIntervention(existing=null){
    ...
    document.getElementById('iMission').value=x.missionType||'diagnostic';this.bindClientChoice('i',x.clientId,x.siteId);this.assistant.bind(document.getElementById('modal'),()=>({kind:'intervention_setup',client:document.getElementById('iClient')?.selectedOptions?.[0]?.textContent||'',title:document.getElementById('iTitle')?.value||''}));
    ...
    fillTeam();document.getElementById('iStart').onchange=fillTeam;document.getElementById('iEnd').onchange=fillTeam;
    ...
    form.onsubmit=async ev=>{ev.preventDefault();await this.saveWithFeedback(form,'Intervention enregistrée ✓',async()=>{
      this.validateFormScheduling('i',x);const {client:c,site:s}=await this.resolveClientChoice('i');x.clientId=c?.id||'';x.siteId=s?.id||'';x.clientName=c?.name||'';x.siteAddress=s?.address||'';
      x.missionType=document.getElementById('iMission').value;x.billingAgreement=document.getElementById('iAgreement').value.trim();x.title=document.getElementById('iTitle').value.trim();x.request=document.getElementById('iRequest').value.trim();x.plannedStart=this.toISO(document.getElementById('iStart').value);x.plannedEnd=this.toISO(document.getElementById('iEnd').value);x.teamSize=Number(document.getElementById('iSize').value)||1;x.vehicle=document.getElementById('iVehicle').checked;x.teamIds=[...document.querySelectorAll('[name=teamPerson]:checked')].map(i=>i.value);if(!['terminee','facturee'].includes(x.status))x.status=x.plannedStart?'planifiee':'a_planifier';x.updatedAt=new Date().toISOString();
  ...
  async openIntervention(id){
    ...
    this.c.modal(`${x.number} · ${x.clientName||'Intervention'}`,`<div class="detailGrid">
      ...
      <div class="formGrid compactReport">
        <div><label>Arrivée réelle</label><input id="aStart" type="datetime-local" value="${this.localDT(x.actualStart)}"></div>
    ...
    bindReportAI(this,x,canEdit);if(canEdit)this.addDraftButton(x,false);
    ...
    if(canEdit){
      ...
      const form=document.getElementById('finishInt');form.onsubmit=async ev=>{ev.preventDefault();await this.saveWithFeedback(form,'Compte rendu enregistré ✓',async()=>{
        this.validateActualHours('aStart','aEnd','aPause');x.actualStart=this.toISO(document.getElementById('aStart').value);x.actualEnd=this.toISO(document.getElementById('aEnd').value);x.pauseHours=Number(document.getElementById('aPause').value)||0;x.workDone=document.getElementById('aWork').value.trim();x.materials=document.getElementById('aMaterial').value.trim();x.notes=document.getElementById('aNotes').value.trim();x.needsQuote=document.getElementById('aQuote').value==='oui'||x.reportItems.some(item=>item.needsQuote);if(x.status!=='facturee')x.status='terminee';x.completedAt=new Date().toISOString();x.updatedAt=new Date().toISOString();await this.save('interventions');try{await this.saveInterventionPackage(x);}catch(e){console.error('PDF auto-save',e);this.c.toast('Rapport enregistré. Le PDF n’a pas pu être archivé automatiquement : utilisez « Enregistrer PDF dans OPUS ».');}
      },async()=>this.renderInterventions());};
    }
  ...
  async openVisit(id){
    ...
    this.c.modal(`${v.number} · ${v.type}`,`<div class="detailGrid"><div><strong>Client</strong><span>${esc(v.clientName||'—')}</span></div><div><strong>Adresse</strong><span>${esc(v.siteAddress||'—')}</span></div><div><strong>Téléphone</strong><span>${esc(v.phone||'—')}</span></div><div><strong>E-mail</strong><span>${esc(v.email||'—')}</span></div><div><strong>Date</strong><span>${esc(v.plannedStart?frDateTime(v.plannedStart):'À planifier')}</span></div><div><strong>Demande</strong><span>${esc(v.request||'—')}</span></div></div><hr><div class="languageBar">${this.assistant.languageSelectHtml()}<span>🎙 Dictez pendant la visite, puis générez une formulation française propre.</span></div><form id="finishVisit">
      ...
      <div class="actionRow">${canEdit?'<button type="submit">Terminer et enregistrer la visite</button>':''}<button type="button" id="downloadVisitPdf" class="secondary">Télécharger le PDF</button><button type="button" id="saveVisitPdf" class="secondary">Enregistrer PDF dans OPUS</button>${this.canPlan()?'<button type="button" id="editVisit" class="secondary">Modifier / planifier</button>':''}${this.c.isAdmin()?'<button type="button" id="deleteVisitReport" class="danger">Supprimer</button>':''}${this.c.isAdmin()&&!v.convertedProjectId?'<button type="button" id="convertVisit" class="secondary">Transformer en chantier</button>':''}</div></form>`);
    document.getElementById('vNeedsQuote').value=v.needsQuote===false?'non':'oui';bindReportAI(this,v,canEdit,true);if(canEdit)this.addDraftButton(v,true);this.bindPhotoInputs(v.photos,{captureId:'visitPhotoCapture',galleryId:'visitPhotoGallery',previewId:'visitPhotoPreview',maxPhotos:8});this.assistant.bind(document.getElementById('modal'),()=>({kind:'visit',client:v.clientName,address:v.siteAddress,type:v.type,number:v.number}));
    if(canEdit){
  ...
  async planEvent(existing=null){
    ...
    const form=document.getElementById('eventForm');form.onsubmit=async ev=>{ev.preventDefault();await this.saveWithFeedback(form,existing?'Planning modifié ✓':'Planning enregistré ✓',async()=>{
      ...
      this.validateScheduling(date,start,end,selectedTeamIds,e.id);
  ...
  async syncEntityFromEvent(e){if(e.kind==='intervention'){const x=this.data.interventions.find(x=>x.id===e.linkId);if(x){x.plannedStart=e.start;x.plannedEnd=e.end;x.teamIds=e.teamIds;x.teamSize=e.teamSize;x.vehicle=e.vehicle;if(!['terminee','facturee'].includes(x.status))x.status='planifiee';await this.save('interventions');}}if(e.kind==='visit'){const x=this.data.visits.find(x=>x.id===e.linkId);if(x){x.plannedStart=e.start;x.plannedEnd=e.end;x.teamIds=e.teamIds;if(!['terminee','convertie'].includes(x.status))x.status='planifiee';await this.save('visits');}}}

  /* ---------- BUREAU ADMIN ---------- */
  async renderOffice(){const root=document.getElementById('officePage'),bill=this.data.interventions.filter(x=>x.status==='terminee'&&!x.projectId);root.innerHTML=`<div class="pageHeading"><div><p class="eyebrow">ESPACE GÉRANT · NON VISIBLE DES TECHNICIENS</p><h1>Bureau OPUS</h1><p class="muted">Facturation, clients, personnel et export mensuel des heures.</p></div></div><div class="adminGrid"><section class="panel"><h2>Observations / devis complémentaires</h2>${this.data.interventions.filter(x=>x.needsQuote||(x.reportItems||[]).some(i=>i.needsQuote)).map(x=>`<div class="row"><div><h3>${esc(x.projectName||x.clientName)} · ${esc(x.number)}</h3>${(x.reportItems||[]).filter(i=>i.needsQuote).map(i=>`<p><strong>${esc(i.title)}</strong> : ${esc(i.observation||i.action||'À examiner')}</p>`).join('')}</div><button data-review-report="${esc(x.id)}">Examiner le rapport</button></div>`).join('')||'<p>Aucun devis complémentaire signalé.</p>'}</section><section class="panel"><div class="panelHead"><h2>À facturer <span class="countBadge">${bill.length}</span></h2></div><div id="billingList">${bill.length?bill.map(x=>`<div class="row"><div><h3>${esc(x.number)} · ${esc(x.clientName)}</h3>${x.billingAgreement?`<p>Accord / référence : ${esc(x.billingAgreement)}</p>`:''}<p>${esc(x.workDone||x.title)} · ${esc(durationHours(x.actualStart||x.plannedStart,x.actualEnd||x.plannedEnd,x.pauseHours).toFixed(2))} h</p></div><button data-bill="${esc(x.id)}">Marquer facturé</button></div>`).join(''):'<p class="muted">Aucune intervention terminée en attente de facturation.</p>'}</div></section><section class="panel"><div class="panelHead"><h2>Paie / heures</h2></div><label>Mois</label><input id="payMonth" type="month" value="${new Date().toISOString().slice(0,7)}"><div class="actionRow"><button id="exportPayroll">Exporter le récapitulatif paie (CSV pour Excel)</button></div><p class="muted">Récapitulatif préparatoire : horaires réalisés et planifiés séparés, congés, maladie, école et fermetures. Vérifiez les lignes à compléter. Les congés à décompter, majorations et retenues sont à valider par la paie.</p></section><section class="panel"><div class="panelHead"><h2>Clients & sites</h2><div class="actionRow compactActions"><button id="importClients" class="secondary">Importer la base clients</button><button id="addClient">+ Client</button></div></div><p class="muted"><strong>${this.data.clients.length}</strong> client(s) dans la base privée Microsoft 365.</p><div id="clientList"></div></section><section class="panel"><div class="panelHead"><h2>Personnel & prestataires</h2><button id="managePeople">Gérer</button></div><div id="peopleSummary"></div></section><section class="panel"><h2>Connecteurs</h2><p><strong>Rexel</strong> — à venir : rapprochement des commandes par numéro d’affaire et dépôt automatique du bon de commande dans le chantier.</p><p><strong>ENEDIS / NDIS</strong> — à venir : constitution guidée du dossier complet.</p></section></div>`;root.querySelectorAll('[data-review-report]').forEach(b=>b.onclick=()=>this.openIntervention(b.dataset.reviewReport));root.querySelectorAll('[data-bill]').forEach(b=>b.onclick=()=>this.markBilled(b.dataset.bill));document.getElementById('exportPayroll').onclick=()=>this.exportPayroll(document.getElementById('payMonth').value);document.getElementById('addClient').onclick=()=>this.editClient();document.getElementById('importClients').onclick=()=>this.importClients();document.getElementById('managePeople').onclick=()=>this.peopleManager();this.renderClientList();this.renderPeopleSummary();}
  async markBilled(id){const x=this.data.interventions.find(x=>x.id===id);if(!x)return;const n=prompt('Numéro de facture (facultatif) :','');if(n===null)return;x.status='facturee';x.invoiceNumber=n||'';x.invoicedAt=new Date().toISOString();await this.save('interventions');await this.renderOffice();}
  importClients(){
  ...
  captureReportDraft(x,visit=false){
    if(!x)return;const form=document.getElementById(visit?'finishVisit':'finishInt');if(!form)return;
  ...
  addDraftButton(x,visit){
    const form=document.getElementById(visit?'finishVisit':'finishInt'),b=document.createElement('button');b.type='button';b.className='secondary';b.textContent='Enregistrer le brouillon';form.querySelector('.actionRow').prepend(b);
    b.onclick=async()=>{b.disabled=true;try{this.captureReportDraft(x,visit);x.updatedAt=new Date().toISOString();await this.save(visit?'visits':'interventions');this.c.toast('Brouillon enregistré. Le statut du rendez-vous est conservé.');}catch(e){this.c.toast('Brouillon non synchronisé : '+e.message);}finally{b.disabled=false;}};
  }
  ...
  validateScheduling(date,start,end,team,exclude=''){
    ...
    for(const id of team){const p=this.personById(id);if(personUnavailable(p,date).unavailable&&!blocked.length)blocked.push((p?.name||'Ressource')+' : indisponible');if(eventConflicts(id,date,start,end,this.data.events,exclude))blocked.push((p?.name||'Ressource')+' : déjà affecté');}
    if(blocked.length)throw new Error(blocked.join(' · ')+'. Modifiez la date ou l’équipe.');
  }
