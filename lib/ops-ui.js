...
export class OpsUI{
  ...
  openProjectReports(p,meta,opts){return projectReports(this,p,meta,opts);}
  ...
  async renderInterventions(){
    const root=document.getElementById('interventionsPage');const rows=this.data.interventions.filter(x=>!x.reportDeletedAt).sort((a,b)=>String(a.plannedStart||'9999').localeCompare(String(b.plannedStart||'9999')));
  ...
  async openIntervention(id){
    ...
    if(x.projectId&&canEdit)document.getElementById('finishInterventionButton').textContent='Envoyer le compte rendu au bureau';
    bindReportAI(this,x,canEdit);if(canEdit)this.addDraftButton(x,false);simplifyReport(x);
