...
import {bindReportAI,reportSource} from './report-ai.js?v=3.4.2';
...
export class OpsUI{
  ...
  async interventionPdf(x,{visit=false}={}){
    ...
    const hasApproved=x.clientReportApproved&&x.clientReport&&x.clientReportSource===JSON.stringify(reportSource(x,savedReadForHeader));
    ...
    const approved=x.clientReportApproved&&x.clientReport&&x.clientReportSource===JSON.stringify(reportSource(x,savedRead));
    if(visit){
  ...
  async visitPdf(v){return this.interventionPdf({...v,clientReportApproved:false},{visit:true});}
  visitExportSnapshot(v){
    const read=(id,fallback)=>document.getElementById(id)?.value??fallback;
    const actual=(id,value,planned)=>{const field=read(id,this.localDT(value||planned));return field===this.localDT(value||planned)?value:this.toISO(field);};
    return {...structuredClone(v),actualStart:actual('vActualStart',v.actualStart,v.plannedStart),actualEnd:actual('vActualEnd',v.actualEnd,v.plannedEnd),pauseHours:Number(read('vPause',v.pauseHours||0))||0,summary:read('vSummary',v.summary||'').trim(),notes:read('vNotes',v.notes||'').trim(),needsQuote:read('vNeedsQuote',v.needsQuote===false?'non':'oui')==='oui'};
  }
  ...
  async openVisit(id){
    ...
    this.c.modal(`${v.number} · ${v.type}`,`<div class="detailGrid"><div><strong>Client</strong><span>${esc(v.clientName||'—')}</span></div><div><strong>Adresse</strong><span>${esc(v.siteAddress||'—')}</span></div><div><strong>Téléphone</strong><span>${esc(v.phone||'—')}</span></div><div><strong>E-mail</strong><span>${esc(v.email||'—')}</span></div><div><strong>Date</strong><span>${esc(v.plannedStart?frDateTime(v.plannedStart):'À planifier')}</span></div><div><strong>Demande</strong><span>${esc(v.request||'—')}</span></div></div><hr><div class="languageBar">${this.assistant.languageSelectHtml()}<span>🎙 Dictez pendant la visite, puis générez une formulation française propre.</span></div><form id="finishVisit">
      ...
      <div class="helpCard"><strong>Aide de saisie FR</strong><p>Utilisez « Français propre » pour reformuler vos notes avant l’export. Le PDF reprend les champs affichés et les photos. Le bouton « Enregistrer PDF dans OPUS » archive la fiche et le PDF dans VISITES_AVANT_DEVIS.</p></div>
      <div class="actionRow">${canEdit?'<button type="submit">Enregistrer la fiche visite</button>':''}<button type="button" id="downloadVisitPdf" class="secondary">Télécharger le PDF</button><button type="button" id="saveVisitPdf" class="secondary">Enregistrer PDF dans OPUS</button>${this.canPlan()?'<button type="button" id="editVisit" class="secondary">Modifier / planifier</button>':''}${this.canPlan()?'<button type="button" id="deleteVisitReport" class="danger">Supprimer</button>':''}${this.c.isAdmin()&&!v.convertedProjectId?'<button type="button" id="convertVisit" class="secondary">Transformer en chantier</button>':''}</div></form>`);
