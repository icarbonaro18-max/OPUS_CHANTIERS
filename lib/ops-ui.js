...
export class OpsUI{
  ...
  async interventionPdf(x){
    ...
    const txt=(value,size=10,bold=false,indent=0)=>{const s=String(value||'').trim();if(!s)return;pdf.setFont('OpusSans',bold?'bold':'normal');pdf.setFontSize(size);const lines=pdf.splitTextToSize(s,contentW-indent);for(const row of lines){ensure(size*.42+1);pdf.text(row,M+indent,y);y+=size*.42;}y+=3;};
    const line=()=>{ensure(5);pdf.setDrawColor(210);pdf.line(M,y,W-M,y);y+=5;};
    const addPhoto=(dataUrl,label='Photo')=>{if(!dataUrl)return;try{const prop=pdf.getImageProperties(dataUrl),maxW=82,maxH=55,ratio=Math.min(maxW/prop.width,maxH/prop.height),w=prop.width*ratio,h=prop.height*ratio;ensure(h+10);pdf.setFontSize(8);pdf.setFont('OpusSans','bold');pdf.text(label,M,y);y+=4;pdf.addImage(dataUrl,'JPEG',M,y,w,h,undefined,'FAST');y+=h+6;}catch(e){console.warn('Photo PDF ignorée',e);}};
    pdf.setFont('OpusSans','bold');pdf.setFontSize(18);pdf.text('OPUS ELEC - RAPPORT D\'INTERVENTION',M,y);y+=9;
    pdf.setFontSize(12);pdf.text(x.number||'Intervention',M,y);y+=7;line();
    txt(`Client : ${x.clientName||'—'}`,11,true);txt(`Adresse : ${x.siteAddress||'—'}`);if(!x.clientReportApproved)txt(`Objet : ${x.title||x.request||'—'}`);txt(`Techniciens : ${this.teamNames(x.teamIds)}`);txt(`Date : ${x.actualStart?frDateTime(x.actualStart):(x.plannedStart?frDateTime(x.plannedStart):'—')}`);
    ...
    if(approved){line();txt('COMPTE RENDU',12,true);txt(x.clientReport);(x.reportItems||[]).forEach((it,i)=>(it.photos||[]).forEach((ph,j)=>addPhoto(ph.dataUrl,`Photo du point ${i+1}.${j+1}`)));}
    else {
    ...
    const pages=pdf.getNumberOfPages();for(let i=1;i<=pages;i++){pdf.setPage(i);pdf.setFont('OpusSans','normal');pdf.setFontSize(8);pdf.setTextColor(110);pdf.text(`OPUS ELEC · ${x.number||''} · page ${i}/${pages}`,M,292);pdf.setTextColor(0);}
