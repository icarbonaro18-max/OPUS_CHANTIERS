...
import {finishAndExport,recordArrival,suggestPlannedEnd} from './intervention-flow.js';
...
export class OpsUI{
  ...
  async save(kind){await this.repo.save(kind,this.data[kind]);if(['events','interventions','projectTracking'].includes(kind)&&this.c.syncProjectStages)try{await this.c.syncProjectStages();}catch(e){this.c.toast('Données enregistrées, classement non synchronisé : '+e.message);}}
  ...
  async editIntervention(existing=null){
    ...
    form.onsubmit=async ev=>{ev.preventDefault();await this.saveWithFeedback(form,'Intervention enregistrée ✓',async()=>{
      suggestPlannedEnd(this);this.validateFormScheduling('i',x);const {client:c,site:s}=await this.resolveClientChoice('i');x.clientId=c?.id||'';x.siteId=s?.id||'';x.clientName=c?.name||'';x.siteAddress=s?.address||'';
  ...
  async openIntervention(id){
    ...
    this.c.modal(`${x.number} · ${x.clientName||'Intervention'}`,`<div class="detailGrid">
      ...
      <div class="actionRow">${canEdit?'<button type="submit" id="finishInterventionButton">Terminer et enregistrer dans OPUS</button>':''}<button type="button" id="downloadIntPdf" class="secondary">Télécharger le PDF</button><button type="button" id="saveIntPdf" class="secondary">Enregistrer PDF dans OPUS</button>${this.canPlan()?'<button type="button" id="editInt" class="secondary">Modifier / planning</button>':''}${this.canPlan()?'<button type="button" id="deleteIntReport" class="danger">Supprimer</button>':''}</div>
    </form>`);
    ...
    if(canEdit){
      ...
      const form=document.getElementById('finishInt');form.onsubmit=async ev=>{ev.preventDefault();await finishAndExport(this,x);};
    }
  ...
  async planEvent(existing=null){
    ...
    const form=document.getElementById('eventForm');form.onsubmit=async ev=>{ev.preventDefault();await this.saveWithFeedback(form,existing?'Planning modifié ✓':'Planning enregistré ✓',async()=>{
      ...
      if(!existing&&(kind==='project'||document.getElementById('eventUntil')?.value||document.querySelector('.extraPeriod'))){
        const base={...e,kind,linkId,teamSize:Number(document.getElementById('eventSize').value)||1,vehicle:document.getElementById('eventVehicle').checked};base.title=this.linkLabel(base);
        ...
        await savePeriods(this,base,periods);return;
      }
      ...
      e.laborHours=durationHours(e.start,e.end,e.pauseHours)*(Number(e.teamSize)||1);e.title=this.linkLabel(e);if(!existing&&!this.data.events.some(x=>x.id===e.id))this.data.events.push(e);await this.save('events');await this.syncEntityFromEvent(e);
  ...
  async upsertLinkedEvent(kind,x){if(!x.plannedStart&&!x.plannedEnd){const previous=this.data.events;this.data.events=previous.filter(e=>!(e.kind===kind&&e.linkId===x.id));if(this.data.events.length!==previous.length){try{await this.save('events');}catch(e){this.data.events=previous;throw e;}}return;}if(!x.plannedStart||!x.plannedEnd)throw Error('Renseignez le début et la fin prévus pour le calendrier.');let e=this.data.events.find(e=>e.kind===kind&&e.linkId===x.id);if(!e){e={id:makeId('evt'),kind,linkId:x.id,teamIds:x.teamIds||[],teamSize:x.teamSize||1,vehicle:x.vehicle||false,pauseHours:x.pauseHours??(durationHours(x.plannedStart,x.plannedEnd,0)>5?1:0),status:'planifie'};this.data.events.push(e);}e.start=x.plannedStart;e.end=x.plannedEnd;e.teamIds=x.teamIds||e.teamIds||[];e.title=kind==='intervention'?`${x.number} · ${x.clientName}`:`${x.number} · ${x.clientName}`;await this.save('events');}
