...
export class OpsUI{
  ...
  interventionCard(x){const st=({a_planifier:'À planifier',planifiee:'Planifiée',en_cours:'En cours',terminee:'Achevée',facturee:'Facturée',annulee:'Annulée'})[x.status]||x.status;return `<article class="projectCard"><div><div class="chips"><span class="chip">${esc(x.number)}</span><span class="chip ${x.status==='terminee'?'warn':x.status==='facturee'?'good':''}">${x.archivedAt?'Archivée':esc(st)}</span></div><h3>${esc(x.clientName||'Client à compléter')} · ${esc(x.title||'Intervention')}</h3><p>${esc(x.siteAddress||'Adresse à compléter')}${x.plannedStart?' · '+esc(frDateTime(x.plannedStart)):''}</p><div class="chips"><span class="chip">${esc(this.teamNames(x.teamIds))}</span></div></div><button data-int="${esc(x.id)}">Ouvrir →</button></article>`;}
