...
export class OpsUI{
  ...
  async renderInterventions(){
    ...
    root.innerHTML=`<div class="pageHeading"><div><p class="eyebrow">PASSAGES PONCTUELS · SAV · DÉPANNAGES</p><h1>Interventions</h1><p class="muted">Chaque intervention reçoit un numéro chronologique et reste liée au client, au planning et à sa fiche terrain.</p></div>${this.canPlan()?'<button id="newIntervention">+ Nouvelle intervention</button>':''}</div>
      <div class="tabs" aria-label="Classement des interventions"><button type="button" id="intCurrent">Interventions en cours (${visible.filter(x=>!['terminee','facturee'].includes(x.status)).length})</button><button type="button" id="intCompleted">Interventions achevées (${visible.filter(x=>['terminee','facturee'].includes(x.status)).length})</button></div>
    ...
    const draw=()=>{const q=norm(document.getElementById('intSearch').value);const completed=this.interventionView==='completed';for(const [id,active] of [['intCurrent',!completed],['intCompleted',completed]]){const b=document.getElementById(id);b.classList.toggle('selected',active);b.setAttribute('aria-pressed',String(active));}const list=visible.filter(x=>['terminee','facturee'].includes(x.status)===completed&&norm([x.number,x.clientName,x.siteAddress,x.title].join(' ')).includes(q));document.getElementById('intList').innerHTML=list.length?list.map(x=>this.interventionCard(x)).join(''):'<div class="panel empty">Aucune intervention.</div>';root.querySelectorAll('[data-int]').forEach(b=>b.onclick=()=>this.openIntervention(b.dataset.int));};
    document.getElementById('intCurrent').onclick=()=>{this.interventionView='current';draw();};document.getElementById('intCompleted').onclick=()=>{this.interventionView='completed';draw();};document.getElementById('intSearch').oninput=draw;draw();if(this.canPlan())document.getElementById('newIntervention').onclick=()=>this.editIntervention();
  }
  interventionCard(x){const st=({a_planifier:'À planifier',planifiee:'Planifiée',en_cours:'En cours',terminee:'Achevée',facturee:'Facturée',annulee:'Annulée'})[x.status]||x.status;return `<article class="projectCard"><div><div class="chips"><span class="chip">${esc(x.number)}</span><span class="chip ${x.status==='terminee'?'warn':x.status==='facturee'?'good':''}">${esc(st)}</span></div><h3>${esc(x.clientName||'Client à compléter')} · ${esc(x.title||'Intervention')}</h3><p>${esc(x.siteAddress||'Adresse à compléter')}${x.plannedStart?' · '+esc(frDateTime(x.plannedStart)):''}</p><div class="chips"><span class="chip">${esc(this.teamNames(x.teamIds))}</span></div></div><button data-int="${esc(x.id)}">Ouvrir →</button></article>`;}
  ...
  async openIntervention(id){
    ...
    this.c.modal(`${x.number} · ${x.clientName||'Intervention'}`,`<div class="detailGrid">
      ...
      <div class="actionRow">${canEdit?'<button type="submit" id="finishInterventionButton">Terminer et enregistrer dans OPUS</button>':''}<button type="button" id="downloadIntPdf" class="secondary">Télécharger le PDF</button><button type="button" id="saveIntPdf" class="secondary">Enregistrer PDF dans OPUS</button>${this.c.isAdmin()?'<button type="button" id="editInt" class="secondary">Modifier la fiche</button>':''}${this.canPlan()?'<button type="button" id="deleteIntReport" class="danger">Supprimer</button>':''}</div>
    </form>`);
    mountMaterialOrders(this,x);const editTop=document.getElementById('editInt');if(editTop)document.getElementById('finishInt').before(editTop);document.getElementById('aQuote').value=x.needsQuote?'oui':'non';this.bindPhotoInputs(x.photos);this.assistant.bind(document.getElementById('modal'),()=>({kind:'intervention',client:x.clientName,address:x.siteAddress,title:x.title,number:x.number}));
  ...
  async planEvent(existing=null,slot=null){
    ...
    const form=document.getElementById('eventForm');form.onsubmit=async ev=>{ev.preventDefault();await this.saveWithFeedback(form,existing?'Planning modifié ✓':'Planning enregistré ✓',async()=>{
      ...
      if(!existing&&(kind==='project'||document.getElementById('eventUntil')?.value||document.querySelector('.extraPeriod'))){
        const base={...e,exactHours:document.getElementById('eventExactHours').checked,kind,linkId,teamSize:Number(document.getElementById('eventSize').value)||1,vehicle:document.getElementById('eventVehicle').checked};base.title=this.linkLabel(base);
