import {OpsRepository,isoDate,weekStart,addDays,frDate,frDateTime,durationHours,nextInterventionNumber,nextVisitNumber,personUnavailable,eventConflicts,standardDayHours,eventKindLabel,makeId,isWithin,isSchoolDay} from './ops.js?v=3.4.0';
import {OpusAssistant} from './assistant.js?v=3.4.0';
...
export class OpsUI{
  ...
  async openIntervention(id){
    ...
    this.c.modal(`${x.number} · ${x.clientName||'Intervention'}`,`<div class="detailGrid">
      ...
      <div class="actionRow">${canEdit?'<button type="submit">Enregistrer le rapport</button>':''}${this.canPlan()?'<button type="button" id="editInt" class="secondary">Modifier / planning</button>':''}${this.canPlan()?'<button type="button" id="deleteIntReport" class="danger">Supprimer</button>':''}</div>
    </form>`);
