...
export class OpsUI{
  ...
  async init(){
    ...
    document.getElementById('officeNav').hidden=this.role()!=='admin';const ordersNav=document.getElementById('ordersAdminNav');if(ordersNav){ordersNav.hidden=this.role()!=='admin';ordersNav.onclick=()=>{if(this.role()==='admin')window.open(new URL('./commandes/',location.href).href,'opus-commandes');};}this.updateRoleUI();
    try{
  ...
  async planEvent(existing=null){
    ...
    const form=document.getElementById('eventForm');form.onsubmit=async ev=>{ev.preventDefault();await this.saveWithFeedback(form,existing?'Planning modifié ✓':'Planning enregistré ✓',async()=>{
      ...
      e.laborHours=e.teamIds.length?plannedTotal(e,this.data.people):durationHours(e.start,e.end,e.pauseHours)*(Number(e.teamSize)||1);e.title=this.linkLabel(e);if(!existing&&!this.data.events.some(x=>x.id===e.id))this.data.events.push(e);await this.save('events');await this.syncEntityFromEvent(e);
  ...
  validateScheduling(date,start,end,team,exclude=''){
    ...
    for(const id of team){const p=this.personById(id);if(personPlan({start:date+'T'+start,end:date+'T'+end,pauseHours:0},p).hours===0)blocked.push((p?.name||'Ressource')+' : hors horaires contractuels');if(personUnavailable(p,date).unavailable&&!blocked.length)blocked.push((p?.name||'Ressource')+' : indisponible');if(eventConflicts(id,date,start,end,this.data.events,exclude))blocked.push((p?.name||'Ressource')+' : déjà affecté');}
