import {reassignPlanning,movableProject,originalAppointment,planningOriginals,calendarEvents} from './planning-reassign.js';
import {mountVisitTabs,mountVisitFiles,addVisitFiles} from './visit-followup.js';
import {mountReception,isReception,openReception,editReception} from './reception.js';
import {bindVisitLocation,visitContactHtml} from './visit-location.js';
...
export class OpsUI{
  ...
  clientChoiceHtml(prefix,currentClientId=''){
    ...
    return `<div><label>Client</label><select id="${prefix}Mode"><option value="existing" ${defaultMode==='existing'?'selected':''}>Client habituel</option><option value="new" ${defaultMode==='new'?'selected':''}>Nouveau client / nouveau site</option></select></div>
      <div id="${prefix}Existing" class="wide"><div class="formGrid">
        ...
        <div><label>${['i','v'].includes(prefix)?'Site connu (facultatif)':'Site / adresse'}</label><select id="${prefix}Site"></select></div>
      </div></div>
      <div id="${prefix}New" class="wide" hidden><div class="formGrid newClientBox">
        ...
        <div class="wide"><label>${['i','v'].includes(prefix)?'Adresse du client / agence':'Adresse du site'}</label><input id="${prefix}Address" placeholder="N°, rue, code postal, ville"></div>
  ...
  async resolveClientChoice(prefix){
    ...
    if(!address)throw new Error('Indiquez l’adresse du client.');
    ...
    if(prefix==='v')Object.assign(site,{address:document.getElementById('vMeetingAddress').value.trim(),contact:document.getElementById('vContact').value.trim(),phone:document.getElementById('vContactPhone').value.trim(),email:document.getElementById('vContactEmail').value.trim()});
    const c={id:makeId('cli'),name:display,category:kind,lastName:individual?name:'',firstName:individual?first:'',phone,email,billingAddress:['i','v'].includes(prefix)?address:'',sites:[site],createdAt:new Date().toISOString()};
  ...
  interventionCard(x){const st=({a_planifier:'À planifier',planifiee:'Planifiée',en_cours:'En cours',terminee:'Terminée',facturee:'Facturée',annulee:'Annulée'})[x.status]||x.status;return `<article class="projectCard"><div><div class="chips"><span class="chip">${esc(x.number)}</span><span class="chip ${x.status==='terminee'?'warn':x.status==='facturee'?'good':''}">${x.archivedAt?'Achevée':esc(st)}</span></div><h3><button type="button" class="textButton" data-int="${esc(x.id)}" style="text-align:left;padding:0;white-space:normal;overflow-wrap:anywhere">${esc(x.clientName||'Client à compléter')} · ${esc(x.title||'Intervention')}</button></h3>${x.request?`<p style="white-space:pre-wrap;overflow-wrap:anywhere">${esc(x.request)}</p>`:''}<p>${esc(x.siteAddress||'Adresse à compléter')}${x.plannedStart?' · '+esc(frDateTime(x.plannedStart)):''}</p><div class="chips"><span class="chip">${esc(this.teamNames(x.teamIds))}</span></div></div><div class="actionRow">${this.c.isAdmin()&&['terminee','facturee'].includes(x.status)?`<button type="button" class="secondary" data-archive-int="${esc(x.id)}" data-restore="${!!x.archivedAt}">${x.archivedAt?'Remettre dans les terminées':'Valider et achever'}</button>`:''}</div></article>`;}
  async editIntervention(existing=null){
    ...
    if(existing&&this.canPlan())document.getElementById('deleteInt').onclick=async()=>{if(!confirm(`Supprimer ${x.number} ?`))return;this.data.interventions=this.data.interventions.filter(r=>r.id!==x.id);this.data.events=reassignPlanning(this.data.events,[],planningOriginals(this.data.events).filter(r=>r.kind==='intervention'&&r.linkId===x.id).map(r=>r.id),this.data.people);await Promise.all([this.save('interventions'),this.save('events')]);document.getElementById('modal').close();await this.renderInterventions();this.c.toast('Intervention supprimée.');};
  }
  async openIntervention(id){
    ...
    this.c.modal(`${x.number} · ${x.clientName||'Intervention'}`,`<div class="detailGrid">
      ...
      <div><strong>Équipe</strong><span>${esc(this.teamNames(x.teamIds))}</span></div><div><strong>Statut</strong><span>${esc(x.archivedAt?'Achevée':this.interventionStatusLabel(x.status))}</span></div>
    ...
    if(canEdit){const hint=document.createElement('p');hint.className='reportOutcome';hint.textContent=x.projectId?'Le rapport sera enregistré et envoyé au bureau selon le type choisi.':'Le rapport sera enregistré puis l’intervention passera dans « Interventions terminées ».';document.getElementById('finishInterventionButton').before(hint);}
  ...
  async renderVisits(){
    ...
    mountVisitTabs(this,root,rows);mountReception(this,root);
  ...
  async editVisit(existing=null){
    if(existing&&isReception(existing))return editReception(this,existing);
    if(existing?!this.c.isAdmin():!this.canPlan())return;
    ...
    this.c.modal(existing?'Modifier la visite':'Nouvelle visite',`<form id="visitForm"><div class="languageBar">${this.assistant.languageSelectHtml()}<span>🎙 Dictée rapide de la demande client.</span></div><div class="formGrid">
      ...
      <div class="wide"><label for="vMeetingAddress">Adresse du rendez-vous / copropriété</label><textarea id="vMeetingAddress" required placeholder="Nom de la résidence, n°, rue, code postal, ville">${esc(v.siteAddress||'')}</textarea><p class="muted">Lieu de la visite, distinct de l’adresse du syndic. La fiche client reste inchangée.</p><button type="button" class="secondary" id="vCopySiteAddress">Reprendre l’adresse du site sélectionné / client</button></div>
      <div><label for="vContact">Contact sur place (facultatif)</label><input id="vContact" value="${esc(v.siteContact||'')}" placeholder="Gardien, résident, conseil syndical…"></div>
      <div><label for="vContactPhone">Téléphone / SMS du contact</label><input id="vContactPhone" type="tel" value="${esc(v.sitePhone??v.phone??'')}" placeholder="06…"></div>
      <div><label for="vContactEmail">E-mail du contact (facultatif)</label><input id="vContactEmail" type="email" value="${esc(v.siteEmail??v.email??'')}"></div>
      <div class="wide"><label for="vAccess">Accès / bâtiment / consignes (facultatif)</label><textarea id="vAccess">${esc(v.siteAccess||'')}</textarea></div>
      ...
      <div class="wide"><label>Documents pour préparer le rendez-vous (PDF / photos, 20 Mo par fichier)<input id="vPreparationFiles" type="file" multiple accept=".pdf,.png,.jpg,.jpeg,.webp,.heic,.heif"></label></div>
    ...
    document.getElementById('vType').value=v.type;this.bindClientChoice('v',v.clientId,v.siteId);bindVisitLocation(this);this.assistant.bind(document.getElementById('modal'),()=>({kind:'visit_setup',type:document.getElementById('vType')?.value||'',client:document.getElementById('vClient')?.selectedOptions?.[0]?.textContent||''}));
    const form=document.getElementById('visitForm');form.onsubmit=async ev=>{ev.preventDefault();await this.saveWithFeedback(form,'Visite enregistrée ✓',async()=>{
      this.validateFormScheduling('v',v);const {client:c,site:s}=await this.resolveClientChoice('v');v.type=document.getElementById('vType').value;v.clientId=c?.id||'';v.siteId=s?.id||'';v.clientName=c?.name||'';v.siteAddress=document.getElementById('vMeetingAddress').value.trim();v.siteContact=document.getElementById('vContact').value.trim();v.sitePhone=document.getElementById('vContactPhone').value.trim();v.siteEmail=document.getElementById('vContactEmail').value.trim();v.siteAccess=document.getElementById('vAccess').value.trim();v.phone=v.sitePhone;v.email=v.siteEmail;v.request=document.getElementById('vReq').value.trim();v.plannedStart=this.toISO(document.getElementById('vStart').value);v.plannedEnd=this.toISO(document.getElementById('vEnd').value);if(!['terminee','convertie'].includes(v.status))v.status=v.plannedStart?'planifiee':'a_planifier';v.updatedAt=new Date().toISOString();if(!existing&&!this.data.visits.some(r=>r.id===v.id))this.data.visits.push(v);await this.save('visits');await this.upsertLinkedEvent('visit',v);await addVisitFiles(this,v,[...(document.getElementById('vPreparationFiles')?.files||[])]);
  ...
  async openVisit(id){
    const reception=this.data.visits.find(x=>x.id===id);if(reception&&isReception(reception))return openReception(this,reception);
    ...
    this.c.modal(`${v.number} · ${v.type}`,`<div class="detailGrid"><div><strong>Client</strong><span>${esc(v.clientName||'—')}</span></div><div><strong>Adresse</strong><span>${esc(v.siteAddress||'—')}</span></div>${visitContactHtml(v)}<div><strong>E-mail</strong><span>${esc(v.email||'—')}</span></div><div><strong>Date</strong><span>${esc(v.plannedStart?frDateTime(v.plannedStart):'À planifier')}</span></div><div><strong>Demande</strong><span>${esc(v.request||'—')}</span></div></div><hr><div class="languageBar">${this.assistant.languageSelectHtml()}<span>🎙 Dictez pendant la visite, puis générez une formulation française propre.</span></div><form id="finishVisit">
    ...
    mountVisitFiles(this,v,document.getElementById('finishVisit'));
    ...
    if(canEdit){
      ...
      const form=document.getElementById('finishVisit');form.onsubmit=async ev=>{ev.preventDefault();await this.saveWithFeedback(form,'Fiche visite enregistrée ✓',async()=>{
        this.validateActualHours('vActualStart','vActualEnd','vPause');v.actualStart=this.toISO(document.getElementById('vActualStart').value);v.actualEnd=this.toISO(document.getElementById('vActualEnd').value);v.pauseHours=Number(document.getElementById('vPause').value)||0;v.summary=document.getElementById('vSummary').value.trim();v.notes=document.getElementById('vNotes').value.trim();v.needsQuote=document.getElementById('vNeedsQuote').value==='oui';v.status='terminee';v.completedAt=new Date().toISOString();v.updatedAt=new Date().toISOString();await this.save('visits');this.visitView=v.quoteNumber?'quoted':'finished';
      },async()=>this.renderVisits());};
    }
  ...
  async renderCalendar(){
    ...
    root.innerHTML=`<div class="pageHeading"><div><p class="eyebrow">PLANNING PARTAGÉ</p><h1>Calendrier des interventions</h1><p class="muted">Vue calendrier : chantier, intervention ou visite. Les apprentis à l’école et les absences restent bloquantes. La nouvelle affectation remplace le créneau précédent.</p></div><div class="actionRow">${this.canPlan()?'<button id="planEvent">+ Planifier</button><button id="resourcesBtn" class="secondary">Équipe & ressources</button>':''}</div></div>
      <div class="calendarViewSwitch"><label>Planning <select id="calendarPerson"><option value="">Général · toutes les équipes</option>${this.data.people.filter(p=>p.active!==false||(this.data.events||[]).some(e=>(e.teamIds||[]).includes(p.id))).map(p=>`<option value="${esc(p.id)}" ${this.calendarPersonId===p.id?'selected':''}>${esc(p.name)}</option>`).join('')}</select></label><button id="dayView" class="${this.calendarView==='day'?'active':''}">Jour</button><button id="weekView" class="${this.calendarView==='week'?'active':''}">Semaine</button><button id="monthView" class="${this.calendarView==='month'?'active':''}">Mois</button></div>
    ...
    document.getElementById('calendarPerson').onchange=e=>{this.calendarPersonId=e.target.value;this.renderCalendar();};
  ...
  dayColumn(d){
    const date=isoDate(d),events=calendarEvents(this).filter(e=>isoDate(e.start)===date).sort(byStart),avail=this.availability(date);
  ...
  teamPickerHtml(selected=[],date=isoDate(new Date()),start='',end='',exclude=''){return `<div class="teamPickerHead"><strong>Composer l’équipe</strong><span>Rouge : déjà affecté, sélection possible pour remplacer ce créneau. École et absence : indisponible.</span></div><div class="teamList">${this.data.people.filter(p=>p.active!==false).map(p=>{const u=personUnavailable(p,date),conf=start&&end&&eventConflicts(p.id,date,start,end,this.data.events,exclude,this.data.people);const closed=closureAt(this.data.closures,date);const outside=start&&end&&personPlan({start:date+'T'+start,end:date+'T'+end,pauseHours:0,fridayExtendedIds:[...document.querySelectorAll('[data-friday-extended]:checked')].map(cb=>cb.value)},p).hours===0;const disabled=u.unavailable||closed||(outside&&!document.getElementById('eventExactHours')?.checked);return `<label class="personPick ${disabled?'disabled':''}" ${conf&&!disabled?'style="color:#a52d38;background:#fff0f1;border:1px solid #e9b8bd"':''}><input name="teamPerson" type="checkbox" value="${esc(p.id)}" ${selected.includes(p.id)?'checked':''} ${disabled&&!selected.includes(p.id)?'disabled':''}><span><strong>${esc(p.name)}</strong><small>${p.type==='external'?'Externe':p.role==='apprenti'?'Apprenti':'OPUS'}${p.vehicle?' · véhicule':''}${disabled||conf?' · '+esc(closed?'Entreprise fermée':outside?'Hors horaires contractuels':conf?'Déjà affecté':this.role()==='admin'?u.label:'Indisponible'):''}</small></span></label>`;}).join('')}</div>`;}
  async planEvent(existing=null,slot=null){
    if(existing)existing=originalAppointment(this.data.events,existing);
    ...
    document.querySelectorAll('[data-friday-extended]').forEach(cb=>cb.onchange=redraw);
    ...
    const form=document.getElementById('eventForm');form.onsubmit=async ev=>{ev.preventDefault();await this.saveWithFeedback(form,existing?'Planning modifié ✓':'Planning enregistré ✓',async()=>{
      ...
      const exactHours=document.getElementById('eventExactHours').checked;this.validateScheduling(date,start,end,selectedTeamIds,e.id,exactHours,true,readPlanningAffairs(kind,linkId).fridayExtendedIds);e.exactHours=exactHours;
      ...
      e.laborHours=e.teamIds.length?plannedTotal(e,this.data.people):durationHours(e.start,e.end,e.pauseHours)*(Number(e.teamSize)||1);e.title=this.linkLabel(e);const previousEvents=this.data.events;this.data.events=reassignPlanning(previousEvents,[e],[],this.data.people);try{await this.save('events');}catch(error){this.data.events=previousEvents;throw error;}if(existing)Object.assign(existing,this.data.events.find(row=>row.id===e.id)||e);await this.syncEntityFromEvent(e);await confirmPlanning(this,[e]);
    },async()=>{await this.renderCalendar();if(this.planningNotice)this.c.toast(this.planningNotice.message);});};
    if(existing)document.getElementById('deleteEvent').onclick=async()=>{if(!confirm('Retirer cette intervention du planning ?'))return;const before=this.data.events;this.data.events=reassignPlanning(before,[],[e.id],this.data.people);try{await this.save('events');}catch(error){this.data.events=before;throw error;}document.getElementById('modal').close();await this.renderCalendar();this.c.toast('Intervention retirée du planning.');};
  }
  async openEvent(id){return previewEvent(this,id);}
  async upsertLinkedEvent(kind,x){if(kind==='intervention')return saveInterventionDays(this,x);if(!x.plannedStart&&!x.plannedEnd){const previous=this.data.events;this.data.events=reassignPlanning(previous,[],planningOriginals(previous).filter(e=>e.kind===kind&&e.linkId===x.id).map(e=>e.id),this.data.people);if(planningOriginals(previous).some(e=>e.kind===kind&&e.linkId===x.id)){try{await this.save('events');}catch(e){this.data.events=previous;throw e;}}return;}if(!x.plannedStart||!x.plannedEnd)throw Error('Renseignez le début et la fin prévus pour le calendrier.');const before=this.data.events;let e=planningOriginals(before).find(e=>e.kind===kind&&e.linkId===x.id);
 e=e?{...e}:{id:makeId('evt'),kind,linkId:x.id,teamIds:x.teamIds||[],teamSize:x.teamSize||1,vehicle:x.vehicle||false,pauseHours:x.pauseHours??(durationHours(x.plannedStart,x.plannedEnd,0)>5?1:0),status:'planifie'};
 e.start=x.plannedStart;e.end=x.plannedEnd;e.teamIds=x.teamIds||e.teamIds||[];e.title=`${x.number} · ${isReception(x)?x.title:x.clientName}`;
 this.data.events=reassignPlanning(before,[e],[],this.data.people);try{await this.save('events');}catch(error){this.data.events=before;throw error;}}
  ...
  validateScheduling(date,start,end,team,exclude='',exactHours=false,reassign=false,fridayExtendedIds=[]){
    ...
    for(const id of team){const p=this.personById(id);if(!exactHours&&personPlan({start:date+'T'+start,end:date+'T'+end,pauseHours:0,fridayExtendedIds},p).hours===0)blocked.push((p?.name||'Ressource')+' : hors horaires contractuels');if(personUnavailable(p,date).unavailable&&!blocked.length)blocked.push((p?.name||'Ressource')+' : indisponible');if(eventConflicts(id,date,start,end,this.data.events.filter(e=>e.allocationSourceId!==exclude&&!(e.displacedProjects||[]).some(p=>p.id===exclude)&&(!reassign||!movableProject(e))),exclude,this.data.people))blocked.push((p?.name||'Ressource')+' : déjà affecté');}
  ...
  validateFormScheduling(prefix,x){
    ...
    this.validateScheduling(start.slice(0,10),start.slice(11),end.slice(11),prefix==='v'?(x.teamIds||[]):[...document.querySelectorAll('[name=teamPerson]:checked')].map(el=>el.value),linked?.id||'',false,true);
  }
