...
export function projectPriority(events,id,now=new Date()){
 const day=isoDate(now),future=events.filter(e=>e.kind==='project'&&e.linkId===id&&!['annulee','cancelled','terminee'].includes(e.status)&&e.start&&e.end&&isoDate(e.end)>=day).sort((a,b)=>a.start.localeCompare(b.start));
 const today=future.find(e=>isoDate(e.start)<=day&&isoDate(e.end)>=day);
 return {today:!!today,event:today||future[0]||null};
}
export async function projectReports(ui,p,meta){
 const reports=ui.data.interventions.filter(x=>x.projectId===p.id).sort((a,b)=>String(b.createdAt).localeCompare(String(a.createdAt)));
 ui.c.modal('Comptes rendus du chantier',`<p><strong>${esc(p.name)}</strong></p><p>Un rapport par passage. Ajoutez les équipements traités, leurs horaires, les photos et les anomalies à reprendre au bureau.</p><button id="newProjectReport">+ Nouveau compte rendu</button><div>${reports.map(x=>`<div class="row"><div><h3>${esc(x.number)}</h3><p>${esc(frDateTime(x.createdAt))} · ${esc(ui.interventionStatusLabel(x.status))} · ${(x.reportItems||[]).length} équipement(s)</p>${(x.reportItems||[]).some(i=>i.needsQuote)?'<span class="chip warn">Devis complémentaire à prévoir</span>':''}</div><button data-project-report="${esc(x.id)}">Ouvrir</button></div>`).join('')||'<p>Aucun compte rendu pour ce chantier.</p>'}</div>`);
 document.querySelectorAll('[data-project-report]').forEach(b=>b.onclick=()=>ui.openIntervention(b.dataset.projectReport));
