...
export async function projectReports(ui,p,meta,opts={}){
 const reports=ui.data.interventions.filter(x=>x.projectId===p.id&&x.status!=='annulee').sort((a,b)=>String(b.createdAt).localeCompare(String(a.createdAt)));
 ui.c.modal('Comptes rendus du chantier',`<p><strong>${esc(p.name)}</strong></p><p>Touchez un rapport pour le consulter ou le reprendre. Créez un nouveau rapport seulement pour un nouveau passage.</p><label>Créer un rapport de</label><select id="newReportKind">${Object.entries(REPORT_TYPES).map(([k,v])=>`<option value="${k}">${v}</option>`).join('')}</select><button type="button" id="newProjectReport">+ Nouveau rapport</button><div>${reports.map(x=>`<div class="row"><div><h3><button type="button" class="documentLink" data-project-report="${esc(x.id)}">${esc(x.number)} · ${esc(REPORT_TYPES[x.reportKind]||'Compte rendu')}</button></h3><p>${esc(frDateTime(x.actualEnd||x.createdAt))} · ${esc(x.pdfSavedAt?'Rapport et PDF enregistrés':x.reportSubmittedAt?'Compte rendu enregistré · PDF à vérifier':'Brouillon à reprendre')} · ${esc(x.reportSubmittedBy||x.createdBy||'')} · ${(x.photos||[]).length} photo(s) générale(s)</p>${x.materialNeeded?'<p class="warning">Matériel à prévoir'+(x.materialNeededOn?' pour le '+esc(x.materialNeededOn):'')+' : '+esc(x.materialNeeded)+'</p>':''}${(x.reportItems||[]).some(i=>i.needsQuote)?'<span class="chip warn">Devis complémentaire à prévoir</span>':''}</div>${x.pdfFileId?`<button type="button" data-report-pdf="${esc(x.id)}">Voir le PDF enregistré</button>`:''}</div>`).join('')||'<p>Aucun compte rendu pour ce chantier.</p>'}</div>`);
