...
export async function projectReports(ui,p,meta,opts={}){
 ...
 ui.c.modal('Comptes rendus du chantier',`<p><strong>${esc(p.name)}</strong></p><p>Un rapport par passage ou journée : travail réalisé, avancement, photos et matériel nécessaire pour la suite.</p><label>Créer un rapport de</label><select id="newReportKind">${Object.entries(REPORT_TYPES).map(([k,v])=>`<option value="${k}">${v}</option>`).join('')}</select><button id="newProjectReport">+ Créer mon compte rendu</button><div>${reports.map(x=>`<div class="row"><div><h3>${esc(x.number)} · ${esc(REPORT_TYPES[x.reportKind]||'Compte rendu')}</h3><p>${esc(frDateTime(x.actualEnd||x.createdAt))} · ${esc(x.reportSubmittedAt?'Envoyé au bureau':ui.interventionStatusLabel(x.status))} · ${esc(x.reportSubmittedBy||x.createdBy||'')} · ${(x.photos||[]).length} photo(s) générale(s)</p>${x.materialNeeded?'<p class="warning">Matériel à prévoir'+(x.materialNeededOn?' pour le '+esc(x.materialNeededOn):'')+' : '+esc(x.materialNeeded)+'</p>':''}${(x.reportItems||[]).some(i=>i.needsQuote)?'<span class="chip warn">Devis complémentaire à prévoir</span>':''}</div><button data-project-report="${esc(x.id)}">Ouvrir</button></div>`).join('')||'<p>Aucun compte rendu pour ce chantier.</p>'}</div>`);
 ...
 if(opts.create){const b=document.getElementById('newProjectReport');await b.onclick({currentTarget:b});}
}
