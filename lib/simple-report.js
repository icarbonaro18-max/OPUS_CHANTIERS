...
export function simplifyReport(x,visit=false){
 ...
 if(oldDetails){
  ...
  for(const [label,value] of [['Dossier',x.projectName||x.number],['Client',x.clientName],['Lieu d’intervention',x.siteAddress],['Contact sur place',[x.siteContact,x.sitePhone||x.phone,x.siteEmail||x.email].filter(Boolean).join(' · ')]]){
   if(!value)continue;const line=document.createElement('div'),title=document.createElement('strong');title.textContent=label+' : ';line.append(title,document.createTextNode(value));box.append(line);
  }
