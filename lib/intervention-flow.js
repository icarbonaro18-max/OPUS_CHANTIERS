...
function feedback(ui,message){
 let box=document.getElementById('reportSaveStatus');if(!box){box=document.createElement('p');box.id='reportSaveStatus';box.setAttribute('role','status');box.className='hint';document.getElementById('finishInt')?.prepend(box);}box.textContent=message;ui.c.toast(message);
}
export async function finishAndExport(ui,x,{download=false,ask=confirmDeparture}={}){
 ...
 try{
  ...
  if(snapshot.clientReport&&!snapshot.clientReportApproved){feedback(ui,'Relisez et validez le compte rendu français avant de l’exporter.');return;}
  if(!snapshot.workDone&&!snapshot.notes&&!snapshot.clientReport&&!(snapshot.reportItems||[]).length&&!(snapshot.projectId&&(snapshot.progressNotes||snapshot.materialNeeded||snapshot.requestMaterials?.length||snapshot.photos?.length))){feedback(ui,'Ajoutez le compte rendu ou les points de l’intervention avant de terminer.');return;}
  ...
  try{const result=await ui.saveInterventionPackage(x,{download});if(isFinalProjectReport(x)){try{await updateTracking(ui,x.projectId,{}, {finish:true});if(ui.c.ensureProjectReview)await ui.c.ensureProjectReview(x.projectId);else throw Error('Classement à vérifier après actualisation du chantier');}catch(e){feedback(ui,'Rapport et PDF enregistrés, mais passage à contrôler non confirmé : '+e.message+'. Réessayez l’enregistrement.');return;}}await ui.c.clearReportDraft?.(x.id);feedback(ui,(x.projectId?(isFinalProjectReport(x)?'Rapport enregistré. Chantier à contrôler. ':'Suivi enregistré. Chantier laissé en cours. '):'Fin d’intervention validée. ')+'PDF enregistré dans '+result.folder);const status=document.getElementById('clientReportStatus');if(status)status.textContent=isFinalProjectReport(x)?'Rapport enregistré. Chantier à contrôler.':'Compte rendu enregistré dans OPUS.';}
  catch(e){const message='Horaires et rapport enregistrés, mais PDF non archivé dans OPUS : '+e.message+'. Réessayez « Enregistrer PDF dans OPUS ».';feedback(ui,message);const status=document.getElementById('clientReportStatus');if(status)status.textContent=message;}
 }catch(e){feedback(ui,'Enregistrement non terminé : '+e.message+'. Ne fermez pas le rapport ; corrigez ou réessayez.');}
 finally{ui.finishingIntervention=false;buttons.forEach(b=>b.disabled=false);if(document.getElementById('interventionsPage'))try{await ui.renderInterventions();}catch(e){console.warn('Affichage à actualiser',e);}}
}
