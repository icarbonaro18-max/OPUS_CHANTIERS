...
import {addPdfLogo} from './pdf-logo.js';
...
export function fridayExtra(person,day){
 return (person.role==='apprenti'||Number(person.weeklyTarget)===35)&&new Date(day.date+'T12:00:00').getDay()===5&&day.nature==='Travail'?Math.max(0,Number(day.hours||0)-3):0;
}
export function monthSheet(data,personId,month,saved={}){
 ...
 return Array.from({length:days},(_,i)=>{const date=isoDate(new Date(y,m-1,i+1,12)),rows=facts.filter(r=>r.date===date),actual=rows.reduce((n,r)=>n+r.actual,0),fingerprint=JSON.stringify(rows),old=saved.days?.find(d=>d.date===date);return {date,actual,actualSchedule:rows.filter(r=>r.actualStart&&r.actualEnd).map(r=>new Date(r.actualStart).toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'})+'–'+new Date(r.actualEnd).toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'})+' · pause '+r.actualPauseHours+' h').join(' ; '),standard:standardDayHours(person,date),details:rows.map(r=>[r.type,r.ref,r.label].filter(Boolean).join(' · ')).join(' ; '),fingerprint,hours:old?.hours??actual,nature:old?.nature||'Travail',note:old?.note||'',reviewed:!!old?.reviewed&&old.fingerprint===fingerprint,changed:!!old&&old.fingerprint!==fingerprint};});
}
...
export async function sheetPdf(person,month,days){
 ...
 const r=await fetch(new URL('../assets/logo.png',import.meta.url));if(!r.ok)throw Error('Logo OPUS indisponible.');addPdfLogo(p,new Uint8Array(await r.arrayBuffer()),12,10,43,14);
 ...
 const row=(values,h,header=false,highlight=false)=>{let x=12;p.setFont('OpusSans',header?'bold':'normal');p.setFontSize(7.1);for(let i=0;i<values.length;i++){p.setFillColor(...(header?[12,61,99]:highlight?[255,241,170]:[248,250,252]));p.setDrawColor(213,225,234);p.rect(x,y,widths[i],h,'FD');p.setTextColor(...(header?[255,255,255]:[22,52,75]));let v=String(values[i]);if(header){const lines=p.splitTextToSize(v,widths[i]-3);p.text(lines,i<2?x+1.5:x+widths[i]-1.5,y+(h-lines.length*2.7)/2+2,{align:i<2?'left':'right',lineHeightFactor:1.1});}else{while(p.getTextWidth(v)>widths[i]-3&&v.length>1)v=v.slice(0,-2)+'…';p.text(v,i<2?x+1.5:x+widths[i]-1.5,y+h/2+1,{align:i<2?'left':'right'});}x+=widths[i];}y+=h;};
 row(heads,10,true);const totals=Array(6).fill(0);
 for(const d of days){const cells=payrollCells(d);cells.forEach((v,i)=>totals[i]+=v);row([d.date.slice(8)+'/'+d.date.slice(5,7),shortWorkLabel(d.details)||'—',...cells.map(v=>v?v.toFixed(2):'—')],5.7,false,fridayExtra(person,d)>0);}
 ...
 const extras=days.filter(d=>fridayExtra(person,d)>0);
 if(extras.length){
  p.addPage();p.setFontSize(14);p.setTextColor(12,61,99);p.text('VENDREDIS APPRENTI — HEURES EN PLUS',12,20);
  p.setFontSize(10);p.text(person.name+' — '+month,12,29);
  p.setFontSize(8);p.text('Heures supplémentaires proposées au comptable, à valider avec le total hebdomadaire.',12,38);
  p.text('Déjà incluses dans le travail du mois : ne pas les ajouter une seconde fois.',12,44);
  let yy=54;for(const d of extras){
   const lines=p.splitTextToSize(d.date+' · '+Number(d.hours).toFixed(2)+' h travaillées · '+fridayExtra(person,d).toFixed(2)+' h en plus\n'+(d.actualSchedule||'Horaires détaillés non pointés')+(d.note?'\nBureau : '+d.note:''),178);
   const h=lines.length*4+8;if(yy+h>275){p.addPage();yy=20;}
   p.setFillColor(255,241,170);p.rect(12,yy,186,h,'F');p.text(lines,16,yy+6);yy+=h+4;
  }
  p.text('Total signalé : '+extras.reduce((n,d)=>n+fridayExtra(person,d),0).toFixed(2)+' h',12,yy+6);
 }
 for(let page=1;page<=p.getNumberOfPages();page++){p.setPage(page);p.setFillColor(255,255,255);p.rect(10,281,190,10,'F');p.setFontSize(7);p.setTextColor(40,65,85);p.text('OPUS ELEC · Synthèse préparatoire à la paie · '+month+' · '+page+'/'+p.getNumberOfPages(),12,286);}
...
export async function openPayroll(ui,month){
 ...
 body.innerHTML=`<p>« Journée prévue » applique ${person.role==='apprenti'||Number(person.weeklyTarget)===35?'35':'39'} h par semaine, y compris les horaires du vendredi. Contrôlez aussi les jours sans pointage.</p><div class="psTableWrap"><table class="psTable"><thead><tr><th>Jour / activité</th><th>Terrain</th><th>Travail retenu<br>(hors déjeuner)</th><th>Nature</th><th>Motif</th><th>Vérifié</th></tr></thead><tbody>${days.map((d,i)=>`<tr data-day="${i}"><td>${esc(d.date)}<small style="display:block;max-width:300px">${esc(shortWorkLabel(d.details)||d.details.replace(/\b[A-Z0-9]{20,}\b/g,''))}${d.changed?' — SOURCES MODIFIÉES, À REVÉRIFIER':''}</small></td><td>${d.actual.toFixed(2)} h<small style="display:block">${esc(d.actualSchedule||'')}</small></td><td><input data-k="hours" type="number" min="0" max="24" step="0.25" value="${d.hours}" style="width:90px"><button type="button" data-standard="${i}">Journée prévue (${d.standard} h)</button>${(person.role==='apprenti'||Number(person.weeklyTarget)===35)&&new Date(d.date+'T12:00:00').getDay()===5?`<button type="button" data-friday="${i}" style="background:#fff1aa;color:#594300">Confirmer vendredi 8h–16h · pause 1h</button>`:''}<span data-extra-hours style="display:block;background:#fff1aa;color:#594300"></span></td><td><select data-k="nature">${['Travail','École','Congés payés','Maladie','Sans solde','Fermeture à qualifier','Autre'].map(n=>`<option ${n===d.nature?'selected':''}>${n}</option>`).join('')}</select></td><td><textarea data-k="note" rows="2" placeholder="Motif de correction…">${esc(d.note)}</textarea></td><td><input data-k="reviewed" type="checkbox" ${d.reviewed?'checked':''}></td></tr>`).join('')}</tbody></table></div><div class="psFooter"><p id="psTotal"></p><p id="psFeedback" role="status" aria-live="polite"></p><div class="actionRow"><button id="psSave">Enregistrer la fiche</button><button id="psReview" class="secondary">Valider toutes les journées relues</button><button id="psPdf">Enregistrer et exporter le PDF de ce salarié</button></div></div>`;
 const total=()=>{body.querySelectorAll('[data-day]').forEach(tr=>{const h=fridayExtra(person,days[+tr.dataset.day]);tr.querySelector('[data-extra-hours]').textContent=h?'Heures supplémentaires à valider : '+h.toFixed(2)+' h':'';});document.getElementById('psTotal').textContent='Total retenu, toutes natures (hors déjeuner) : '+days.reduce((n,d)=>n+Number(d.hours||0),0).toFixed(2)+' h';};total();
 ...
 body.querySelectorAll('[data-friday]').forEach(b=>b.onclick=()=>{const d=days[+b.dataset.friday],tr=b.closest('tr');d.hours=7;d.nature='Travail';d.note='Vendredi confirmé par le bureau : 8h–16h, pause déjeuner 1h, soit 7h travaillées (4h en plus à valider en paie).';d.reviewed=false;tr.querySelector('[data-k=hours]').value=7;tr.querySelector('[data-k=nature]').value='Travail';tr.querySelector('[data-k=note]').value=d.note;tr.querySelector('[data-k=reviewed]').checked=false;dirty=true;total();});
