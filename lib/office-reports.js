import {openImportReports} from './import-intervention-reports.js';
...
export function openOfficeReports(ui){
 ...
 const draw=()=>{
  ...
  list.querySelectorAll('[data-delete-report]').forEach(b=>b.onclick=async()=>{
   ...
   if(!state.trash&&!confirm('Mettre le compte rendu '+(r.number||'')+' dans la corbeille ? La fiche et ses rendez-vous resteront accessibles.'))return;
   b.disabled=true;await run(async()=>{await setReportDeleted(ui,r,!state.trash,kind==='visit'?'visits':'interventions');root.querySelector('#officeReportNotice').textContent=state.trash?'Compte rendu restauré.':'Compte rendu placé dans la corbeille. La fiche et le planning sont conservés.';draw();await ui.renderOffice();});b.disabled=false;
  });
 };
