...
export function missionHtml(e,x){
 const rows=[['Dossier',x?[x.number,x.clientName].filter(Boolean).join(' · '):e.title],['Objet',x?.title||e.purpose],['Objet de ce passage',x?.title?e.purpose:''],['Demande / travaux prévus',x?.request],['À faire pour ce rendez-vous',e.instructions]].filter(([,v])=>String(v||'').trim());
