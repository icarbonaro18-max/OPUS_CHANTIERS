...
export function personPlan(event,person){
 ...
 if(event.exactHours||!person||person.type==='external'||[0,6].includes(start.getDay())||start.toDateString()!==end.toDateString())return fallback();
 const friday=start.getDay()===5,apprentice=person.role==='apprenti'||Number(person.weeklyTarget)===35;
 const extended=friday&&apprentice&&(event.fridayExtendedIds||[]).includes(person.id);
 const begin=new Date(start),finish=new Date(start);begin.setHours(8,0,0,0);finish.setHours(friday?(apprentice&&!extended?11:16):17,0,0,0);
 const a=new Date(Math.max(+start,+begin)),b=new Date(Math.min(+end,+finish)),gross=Math.max(0,(b-a)/3600000);
 let pause=friday&&apprentice&&!extended?0:start<=begin&&end>=finish?1:Math.max(0,Number(event.pauseHours)||0);pause=Math.min(gross,pause);
