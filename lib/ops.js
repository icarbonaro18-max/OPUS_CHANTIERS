...
export class OpsRepository{
  ...
  async loadAll(){
    const [clients,people,interventions,visits,events,closures]=await Promise.all([
      this.load('clients',[]),this.load('people',[]),this.load('interventions',[]),this.load('visits',[]),this.load('events',[]),this.load('closures',[])
    ]);
    return {clients,people,interventions,visits,events,closures};
  }
}
