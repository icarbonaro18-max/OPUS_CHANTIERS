...
export function payrollRows(data,month){
 ...
 for(const p of data.people.filter(p=>p.type!=='external'))for(let d=new Date(y,m-1,1,12);d<=last;d=addDays(d,1)){
  ...
  for(const e of [...events,...extra]){
   ...
   push({type:eventKindLabel(e.kind),ref:x?.number||e.linkId,label:x?.clientName||e.title||'',actual:h,actualStart:actualPair?.[0]||'',actualEnd:actualPair?.[1]||'',actualPauseHours:actualPair?.[2]||0,planned:personPlan(e,p).hours,alert:absences.length||closed||school?'Chevauchement avec indisponibilité : à vérifier':actualPair?'':'Horaires réels non renseignés'});
  }
