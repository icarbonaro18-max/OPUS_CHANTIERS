...
async function activate(){
 $('start').hidden=true;$('shell').hidden=false;$('userBtn').hidden=false;$('refresh').hidden=false;$('userBtn').textContent=user.displayName;
 await updatePending();await refresh();clearInterval(pollId);
...
function showDashboard(){$('dashboard').hidden=false;$('projectPage').hidden=true;$('allProjects').classList.toggle('active',category==='all');renderProjects();}
...
bind('userBtn',()=>{modal('Votre session OPUS CHANTIERS',`<p><strong>${esc(user.displayName)}</strong><br>${esc(user.mail||user.userPrincipalName||'')}</p><p class="muted">La déconnexion concerne cette application, pas vos OneDrive ni les autres appareils.</p><div class="actionRow"><button id="relogin">Reconnecter</button><button id="logout" class="secondary">Se déconnecter</button></div>`);bind('relogin',async()=>{if(DEMO){$('modal').close();return;}await msal.loginRedirect({scopes:config.scopes});});bind('logout',async()=>{if((await store.all('draft:')).length)throw Error('Des saisies attendent un envoi. Envoyez-les ou sauvegardez-les avant la déconnexion.');if(DEMO){location.href='./';return;}await msal.logoutRedirect({account:msal.getActiveAccount()});});});
async function init(){
