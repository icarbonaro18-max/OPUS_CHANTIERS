import {Graph,Journal,LocalStore,CATEGORIES,uid,norm,safeName,sha,blobData,CloudError,revisionHeads,parseRevision} from './lib/cloud.js?v=3.2.1';
...
async function restoreRoute(){const r=routeState();if(!r.projectId)return;const p=catalog.projects.find(x=>x.id===r.projectId);if(!p){setRoute();return;}await openProject(r.projectId);if(['overview','documents','attestations','tasks'].includes(r.tab)&&r.tab!=='overview')await setTab(r.tab);}
...
async function activate(){
 ...
 try{
  const {OpsUI}=await import('./lib/ops-ui.js?v=3.2.1');
