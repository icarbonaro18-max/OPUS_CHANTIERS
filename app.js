...
import {importSheet} from './lib/chantier-sheet.js';
...
async function openProject(id){
 ...
 if(remoteLoaded&&isAdmin()&&!metaConflict&&!draft){
  try{const next=await autoFillCoordinates(g,p,clone(meta));if(selected?.id!==p.id)return;if(next){await queueSave(p.id,'fiche',null,next,metaParents);meta=next;toast('Client et adresse repris de la feuille de chantier. Vérifiez les coordonnées dans la fiche.');}}
  catch(e){toast('Coordonnées non reprises automatiquement : '+e.message+' Utilisez « Reprendre la feuille PDF ».');}
 }
...
async function importCurrentSheet(){if(!isAdmin())throw Error('Import réservé à l’administrateur.');const p=selected;await importSheet({g,project:p,meta:clone(meta),isAdmin,modal,save:async next=>{if(selected?.id!==p.id)throw Error('Rouvrez le chantier concerné.');await queueSave(p.id,'fiche',null,next,metaParents);meta=next;},done:async()=>{$('modal').close();await openProject(p.id);toast('Reprise conservée. Vérifiez les envois en attente pour la confirmation Microsoft 365.');}});}
...
async function documents(reset=true){
 ...
 bind('addFile',()=>uploadModal(dir));void displayDocumentCounts(g,$('tabContent'));
}
...
async function ensureProjectReview(id){
 ...
 if(!p||!cat)throw Error('Chantier ou rubrique À contrôler introuvable');
 if(!opsUI.data.projectTracking?.find(r=>r.id===id)?.finishedRequested)throw Error('Fin de chantier non enregistrée');
 if(['04','99'].includes(p.category))throw Error('Ce chantier est déjà clôturé ; vérifier son classement au bureau');
