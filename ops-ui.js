...
export class OpsUI{
  ...
  async editVisit(existing=null){
    ...
    document.getElementById('vType').value=v.type;this.bindClientChoice('v',v.clientId,v.siteId);document.getElementById('vCopySiteAddress').onclick=()=>{const c=this.clientById(document.getElementById('vClient').value),site=this.siteById(c,document.getElementById('vSite').value);document.getElementById('vMeetingAddress').value=document.getElementById('vMode').value==='new'?document.getElementById('vAddress').value:(site?.address||c?.billingAddress||'');};this.assistant.bind(document.getElementById('modal'),()=>({kind:'visit_setup',type:document.getElementById('vType')?.value||'',client:document.getElementById('vClient')?.selectedOptions?.[0]?.textContent||''}));
