...
export class Graph {
 constructor(token,fetcher=null){this.token=token;this.fetcher=fetcher||((...args)=>globalThis.fetch(...args));this.drive=null;this.site=null;}
 ...
 async writeJson(parent,name,value){
  const blob=new Blob([JSON.stringify(value,null,2)],{type:'application/json'});
  return this.request(this.base(parent)+':/'+encodeURIComponent(name)+':/content',{method:'PUT',headers:{'Content-Type':'application/json'},body:blob});
 }
 async availableName(parent,name){const dot=name.lastIndexOf('.'),base=dot>0?name.slice(0,dot):name,ext=dot>0?name.slice(dot):'';if(!await this.named(parent,name))return name;for(let i=2;i<1000;i++){const candidate=`${base} (${i})${ext}`;if(!await this.named(parent,candidate))return candidate;}throw new CloudError('Impossible de trouver un nom de fichier disponible.');}
 async upload(parent,name,blob,onProgress=()=>{},immutable=false){
  if(!blob.size)throw new CloudError('Le fichier est vide.');
  let target=name;const existing=await this.named(parent,target);
  if(existing){if(immutable)throw new CloudError('Un fichier de même nom existe déjà.',409);target=await this.availableName(parent,target);}
  // JSON, PDF et photos usuels : envoi simple Microsoft Graph, plus fiable dans les navigateurs mobiles.
  if(blob.size<=4*1024*1024){const item=await this.request(this.base(parent)+':/'+encodeURIComponent(target)+':/content',{method:'PUT',headers:{'Content-Type':blob.type||'application/octet-stream'},body:blob});onProgress(100);return item;}
  // Gros fichiers : session découpée, sans envoyer le jeton sur l'URL préautorisée.
  const session=await this.request(this.base(parent)+':/'+encodeURIComponent(target)+':/createUploadSession',{method:'POST',body:{item:{name:target,'@microsoft.graph.conflictBehavior':immutable?'fail':'rename'}}});
 ...
 async deleteItem(id){return this.request(this.base(id),{method:'DELETE'});}
}
